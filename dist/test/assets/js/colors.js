...
export function applyChangeColor(node,style,legend){
 if(!style)return;
 const matches=labelsFor(style,legend);
 applyColor(node,{
  background:matches.some(item=>!neutral(item.background)&&item.background===style.background)?style.background:null,
  foreground:matches.some(item=>!neutral(item.foreground)&&item.foreground===style.foreground)?style.foreground:null
 });
}
