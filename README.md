...
Google スプレッドシートの「時間割」「連絡」を表示する、スマートフォン対応の静的Webアプリです。現在は **1-1**、**1-5**、**2-1**、**2-3**、**2-5**、**3-1** に対応しています。

PWA対応のため、スマートフォンやパソコンのホーム画面へ追加して利用できます。クラスのページから追加すると、そのクラスの時間割が直接開きます。通信できない場合も、最後に取得した時間割を表示します。
